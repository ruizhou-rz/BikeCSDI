...
def process_func(path: str, missing_ratio=0.1, encode=True):
...
class tabular_Dataset(Dataset):
    ...
    def __init__(self, eval_length=182, use_index_list=None, missing_ratio=0.1, seed=0):
        self.eval_length = eval_length
        np.random.seed(seed)

        dataset_path = "./new_colored_biked.csv"
        processed_data_path = (
            f"./data/missing_ratio-{missing_ratio}_seed-{seed}.pk"
        )
        processed_data_path_norm = f"./data/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, eval_length=100):
    dataset = tabular_Dataset(eval_length=eval_length, missing_ratio=missing_ratio, seed=seed)
    ...
    if not os.path.isfile(processed_data_path_norm):
        ...
        with open("./data/stat.pk", "wb") as f:
            pickle.dump([max_arr, min_arr], f)
    ...
    with open("test_index.pk","wb") as f:
